...
function BadgeTrophyIcon({ size = 18 }: { size?: number }) {
  return <svg width={size} height={size} viewBox="0 0 18 18" fill={C.orange}><path d="M9 1L11 7H17L12.5 10.5L14.5 16.5L9 13L3.5 16.5L5.5 10.5L1 7H7Z"/></svg>;
}
function AnnouncementIcon({ size = 18 }: { size?: number }) {
  return <svg width={size} height={size} viewBox="0 0 18 18" fill={C.blue}><path d="M9 1C6 1 3.5 3.5 3.5 6.5V11L2 13V14H16V13L14.5 11V6.5C14.5 3.5 12 1 9 1Z"/><rect x="7" y="14" width="4" height="3" rx="1.5"/></svg>;
}
function SurveyIcon({ size = 18 }: { size?: number }) {
  return <svg width={size} height={size} viewBox="0 0 18 18" fill={C.purple}><rect x="2" y="1.5" width="14" height="15" rx="2.5"/><rect x="5" y="5" width="8" height="1.5" rx="0.75" fill="white" opacity="0.8"/><rect x="5" y="8.5" width="8" height="1.5" rx="0.75" fill="white" opacity="0.8"/><rect x="5" y="12" width="5" height="1.5" rx="0.75" fill="white" opacity="0.6"/></svg>;
}
function CheckCircleIcon({ color, size = 18 }: { color: string; size?: number }) {
  return <svg width={size} height={size} viewBox="0 0 18 18" fill={color}><circle cx="9" cy="9" r="8.5"/><path d="M4.5 9L7 12L13.5 5.5L12 4.5L7 9.5L6 8L4.5 9Z" fill="white"/></svg>;
}
function GlobeFullIcon({ color, size = 18 }: { color: string; size?: number }) {
  return <svg width={size} height={size} viewBox="0 0 18 18" fill={color}><circle cx="9" cy="9" r="7.5" opacity="0.2"/><path d="M9 1.5C9 1.5 6 4.5 6 9C6 13.5 9 16.5 9 16.5C9 16.5 12 13.5 12 9C12 4.5 9 1.5 9 1.5Z"/><rect x="1.5" y="8" width="15" height="2" rx="1" opacity="0.7"/></svg>;
}
...
function NoticeCard({ n }: { n: NoticeMsg }) {
  ...
  return (
    <div style={{ backgroundColor: C.card, border: "1px solid rgba(0,0,0,0.06)", borderRadius: 22, padding: "16px 18px", boxShadow: "3px 3px 0px rgba(0,0,0,0.10)", marginBottom: 12, borderLeft: n.unread ? `4px solid ${color}` : undefined }}>
      <div style={{ display: "flex", gap: 12, alignItems: "flex-start" }}>
        <div style={{ width: 38, height: 38, borderRadius: 10, backgroundColor: `${color}15`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, marginTop: 2 }}>
          {isBadge ? <BadgeTrophyIcon size={18} /> : isSurvey ? <SurveyIcon size={18} /> : <AnnouncementIcon size={18} />}
        </div>
        <div style={{ flex: 1 }}>
...
function ActivityCard({ a }: { a: ActivityMsg }) {
  ...
  return (
    <div style={{ backgroundColor: C.card, border: "1px solid rgba(0,0,0,0.06)", borderRadius: 22, padding: "16px 18px", boxShadow: "3px 3px 0px rgba(0,0,0,0.10)", marginBottom: 12 }}>
      <div style={{ display: "flex", gap: 12, alignItems: "flex-start" }}>
...
function SectionLabel({ label }: { label: string }) {
  return (
    <p style={{ margin: "0 0 14px", fontSize: 13, color: C.muted, fontWeight: 700, letterSpacing: "0.08em", textTransform: "uppercase", fontFamily: DM_SANS }}>{label}</p>
  );
}
...
export default function ActivityNotifications() {
  return (
    <PageWrapper bg="#EFF3FC">
      <div style={{ flex: 1, overflowY: "auto", paddingBottom: 32, scrollbarWidth: "none" } as React.CSSProperties}>
        <BackHeader title="Activity Notifications" subtitle="Notices · Tracking · Culture" bg="#EFF3FC" />

        <div style={{ padding: "0 24px 8px" }}>
          <SectionLabel label="System Notices" />
          {NOTICES.map(n => <NoticeCard key={n.id} n={n} />)}
        </div>

        <div style={{ padding: "0 24px 8px" }}>
          <SectionLabel label="Activity Tracking" />
          {ACTIVITY_MSGS.map(a => <ActivityCard key={a.id} a={a} />)}
        </div>

        <div style={{ padding: "0 24px 32px" }}>
          <SectionLabel label="Culture & Community" />
          {CULTURE_MSGS.map(c => <CultureCard key={c.id} c={c} />)}
        </div>
      </div>
      <BottomNav active={1} />
    </PageWrapper>
